...
describe('TabList', () => {
  it('shows loading state with spinner text', () => {
    render(<TabList tabs={[]} loading={true} onRestore={vi.fn()} />);
    expect(screen.getByText('Searching...')).toBeInTheDocument();
  });

  it('shows empty state when no tabs and not loading', () => {
    render(<TabList tabs={[]} loading={false} onRestore={vi.fn()} />);
  ...
  it('renders virtualized list when tabs are provided', () => {
    ...
    render(<TabList tabs={tabs} loading={false} onRestore={vi.fn()} />);
  ...
  it('shows loading state even when tabs are provided', () => {
    ...
    const tabs = makeTabs(3);
    render(<TabList tabs={tabs} loading={true} onRestore={vi.fn()} />);
    expect(screen.getByText('Searching...')).toBeInTheDocument();
  });
});
